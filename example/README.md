...
Project customized to meet some concerns of mine like structure, libs, and etc.

## Main changes
- Structure and folders
- Uses axios, moment and material-ui
- Setup some basic services:
  - `api`
  - `session`
  - `genericErrorHandler`
- Setup some basic utils:
  - `query`
- Changed some es-lint validations 
  - Turn off react/multi-comp in single file
  - Allows console warn and error
- Setup tests to mock LocalStorage
- Ready to deploy to Heroku using Express + Server side building (see concerns below)
- VS Code settings with personal config (lots of files excluded, since don't use them too much)

## Services vs Utils

I consider **services** any function or class that: 
- Creates any object or function; or
- Performs some kind of action; or
- Causes any side effect.

I consider **utils** any function or module that:
- Contains only pure functions; or
- Do really simple things that are highly reusable for lots of cases and modules (like formatting number and dates, summing, removing `null`/`undefined` entries from object, etc).

## Concerns about deploy

Don't forget to run `heroku config:set NPM_CONFIG_PRODUCTION=false`, because we need dev dependencies to be included, so Heroku can build it in server side.

API service consider you'll use an API in another host. 
If that's true, you must set your base url in Heroku by running `heroku config:set API_BASE_URL=www.yourapi.com/`, otherwise you may want to remove or customize `process.env.API_BASE_URL` in `webpack.config.[ENV].js` files.
